...
for k in coords:
    ...
    endpt = min(ends, key=lambda x:(x[0]-coords[k]['lat'])**2+(x[1]-coords[k]['lon'])**2) if ends else rotate_bearing(coords[k]["lat"], coords[k]["lon"], bearings[k])
...
# If calculated, show intersection center
if st.session_state.calculated and i_pts:
    ...
    if sel:
        ...
        folium.CircleMarker(int_center, radius=8, color='green', fill=True,
                            fill_color='green', popup=f"Intersection Center\n{int_address if int_address else ''}").add_to(m)
        # Sidebar details
        st.sidebar.header("📍 Intersection Center")
        st.sidebar.markdown(f"🟢 **Coordinates:** {int_center[0]:.6f}, {int_center[1]:.6f}")
        st.sidebar.markdown(f"🟢 **Address:** {int_address if int_address else 'N/A'}")
